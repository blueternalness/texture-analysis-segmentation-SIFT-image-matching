...
// Helper function to match features using Lowe's Ratio test and save the result
void matchAndSave(const string& name1, const Mat& img1, const vector<KeyPoint>& kp1, const Mat& desc1,
                  const string& name2, const Mat& img2, const vector<KeyPoint>& kp2, const Mat& desc2,
                  const string& outputFilename) {
                      
    BFMatcher matcher(NORM_L2);
    vector<vector<DMatch>> knn_matches;
    matcher.knnMatch(desc1, desc2, knn_matches, 2); // Find 2 nearest neighbors
    ...
    cout << "Matched " << name1 << " and " << name2 << " -> Found " 
         << good_matches.size() << " good matches. Saved to " << outputFilename << endl;
}
...
int main() {
    ...
    // --- TASK 1: Find Largest Scale Keypoint in Cat_1 and match in Cat_3 ---
    int largest_idx = 0;
    ...
    // Isolate the single descriptor for the largest keypoint
    Mat single_desc = desc_c1.row(largest_idx);
    
    // Find its nearest neighbor in Cat_3
    ...
    cout << "[Task 1] Largest keypoint in Cat_1:" << endl;
    cout << "  Scale (size): " << kp_c1[largest_idx].size << endl;
    cout << "  Orientation (angle): " << kp_c1[largest_idx].angle << " degrees" << endl;
    
    cout << "[Task 1] Closest neighbor in Cat_3:" << endl;
    cout << "  Scale (size): " << kp_c3[matched_idx_in_c3].size << endl;
    cout << "  Orientation (angle): " << kp_c3[matched_idx_in_c3].angle << " degrees" << endl;
    cout << "------------------------------------------------" << endl;

    // --- TASK 2 & 3: Match all required pairs ---
    // 1) Cat_1 and Cat_3
    matchAndSave("Cat_1", cat1, kp_c1, desc_c1, 
                 "Cat_3", cat3, kp_c3, desc_c3, 
                 "Match_Cat1_Cat3.jpg");

    // 2) Cat_3 and Cat_2
    matchAndSave("Cat_3", cat3, kp_c3, desc_c3, 
                 "Cat_2", cat2, kp_c2, desc_c2, 
                 "Match_Cat3_Cat2.jpg");

    // 3) Dog_1 and Cat_3
    matchAndSave("Dog_1", dog1, kp_d1, desc_d1, 
                 "Cat_3", cat3, kp_c3, desc_c3, 
                 "Match_Dog1_Cat3.jpg");

    // 4) Cat_1 and Dog_1
    matchAndSave("Cat_1", cat1, kp_c1, desc_c1, 
                 "Dog_1", dog1, kp_d1, desc_d1, 
                 "Match_Cat1_Dog1.jpg");
